...
func main() {
	...
	for _, h := range handlers {
		...
		for i, v := range d {
			...
			s += h.Marshal(v)
			if (i+1)%10000 == 0 {
				_, err := es.Bulk(strings.NewReader(s), es.Bulk.WithIndex(h.Index()))
				if err != nil {
					fmt.Println(err)
				} else {
					fmt.Print(".")
...
func (Take2) Marshal(d generator.Data) string {
	return fmt.Sprintf(`{"id":%v,"attributes":["Color:%v", "Size:%v"],"attribute_keys":["Color","Size"]}%v`, d.ID, d.Attributes["Color"], d.Attributes["Size"], "\n")
}
